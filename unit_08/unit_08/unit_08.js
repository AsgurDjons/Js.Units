...
function t1() {
    ...
    while (p < 50) {
        p++;
        document.querySelector('.out-1').textContent += `${p} `;
    }
}
...
function t2() {
    ...
    while (p < 122) {
        p++;
        if (p % 2 == 0) {
            document.querySelector('.out-2').textContent += `${p} `;
        }
    }
}
...
function t3() {
    ...
    while (p > 7) {
        p--;
        document.querySelector('.out-3').textContent += `${p} `;
    }
}
...
function t12() {
    ...
    out = '';
    while (p < divs.length) {
        out += divs[p].style.background = 'orange';
        p++;
    }
